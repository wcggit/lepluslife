...
public class OnLineOrderShareService {
    ...
    public void sMovieOrderShare(SMovieOrder movieOrder) {
        ...
        onLineOrderShare.setToLockMerchant(product.getToMerchant() == null ? 0 : product.getToMerchant());
        onLineOrderShare.setToLockPartner(product.getToPartner() == null ? 0 : product.getToPartner());
        onLineOrderShare.setToLockPartnerManager(product.getToPartnerManager() == null ? 0 : product.getToPartnerManager());
        ...
        if (onLineOrderShare.getShareMoney() > 0) {
            ...
            if (leJiaUser.getBindMerchant() != null && product.getToMerchant() > 0) {
                MerchantWalletOnline merchantWalletOnline = merchantWalletOnlineService.findByMercahnt(leJiaUser.getBindMerchant());
                Merchant merchant = merchantWalletOnline.getMerchant();
                //分润给绑定商户
                ...
                if (merchant.getPartnership() == 2) {     //如果是虚拟商户分润方式改变
                    ...
                    merchantWalletOnlineService.shareToMerchant(product.getToMerchant(), merchant, merchantWalletOnline,
            ...
            if (leJiaUser.getBindPartner() != null && toPartner > 0) {
                ...
                PartnerWalletOnline partnerWalletOnline = partnerWalletOnlineService.findByPartner(leJiaUser.getBindPartner());
            ...
            if (leJiaUser.getBindPartnerManager() != null && product.getToPartnerManager() > 0) {
                ...
                PartnerManagerWalletOnline partnerManagerWalletOnline = partnerManagerOnlineService.findByPartnerManager(leJiaUser.getBindPartnerManager());
                ...
                partnerManagerOnlineService.shareToPartnerManager(product.getToPartnerManager(), leJiaUser.getBindPartnerManager(), partnerManagerWalletOnline, movieOrder.getOrderSid(), 2L);
            }
            onLineOrderShare.setShareMoney(product.getToMerchant() + toPartner + product.getToPartnerManager());
